...
5. `check / lock / tree / vendor / pack / publish / tool install|use|pin / build|run|test --dry-run` 的 machine-readable success payload

## 4. 当前最关键的三项 handoff

1. `ProjectGraphSnapshot` 所需字段的正式 machine contract
2. workflow success payload，而不是只有失败 JSON
3. toolchain / registry / publish preflight 的 machine contract
